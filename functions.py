...
def DROPGROWTHRATE(T,P,RH,RH_EQ,RHOAT,D):
   ...
   # FH, FV = VENTILATION01(D, RHOAT, T, P)
    ...
    return DSTAR*(RH-RH_EQ)*SVP/RAD/DROPGROWTHRATE # satuation ratio*satuation vapour pressure
...
def find_act_mass(aer_mass,T, nbins, nmodes,rhobin, Kappa):
    ...
    def kk02(NW):
        """ Kappa Koehler theory, Petters and Kriedenwies (2007)
    """
        MWAT = NW*c.mw   #NW is the amount of substance 
        RH_ACT = 0
   ...
   # T = output[idx,ITEMP]
    ...
    for i in range(nmodes*nbins):
        ...
        act_mass[i] = minimize_scalar(kk02,bracket=(brac*0.1, brac*50), method='brent', tol=1e-30).x*c.mw  #Note the ".x"
...
def K01SV(MWAT, T, mass_bin_centre,n_sv, rhobin, nubin, molwbin): # Each condensing compound has an equivalent equation to the traditional Kohler equation.
    """ Koehler theory 
...
def SVP_GASES(SV_dict, T, n_sv):   ###Saturation Vapor Pressure of semi-vols
...
def SVGROWTHRATE(T, P, SVP_ORG, RH, RH_EQ,DIAM, n_sv,nbins,nmodes,molwbin): #SVP_ORG: Saturation Vapor Pressure of semi-vols #RH: Relative humidity of semivols (Saturation ratio?) #RH_EQ: K01SV return 的第0个值
